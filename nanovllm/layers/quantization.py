...
class AWQLinear(nn.Module):
    def __init__(self, in_features, out_features, group_size=128, bias=False):
        ...
        # 注册 Buffer
        # 形状定义：PyTorch Linear 权重通常存储为 (Out, In)
        # AWQ 沿 In 维度打包 -> (Out, In // 8)
        ...
        self.register_buffer('qzeros', torch.zeros((out_features, in_features // group_size // 8), dtype=torch.int32))
    ...
    def forward(self, x):
        ...
        return awq_gemm_triton(x,self.qweight,self.scales,self.qzeros,split_k_iters=4)